...
import org.zionusa.admin.dao.activities.ChurchActivityPlanDao;
...
@Service
public class ChurchActivityPlanService extends BaseService<ChurchActivityPlan, Integer> {
    private static final Logger logger = LoggerFactory.getLogger(ChurchActivityPlanService.class);
    private final ActivityCategoryDao activityCategoryDao;
    private final ChurchActivityPlanDao dao;
    private final ChurchActivityReportViewDao reportDao;
    private final SpecialActivityViewService specialActivityViewService;

    @Autowired
    public ChurchActivityPlanService(ActivityCategoryDao activityCategoryDao,
                                     ChurchActivityPlanDao dao,
                                     ChurchActivityReportViewDao reportDao,
                                     SpecialActivityViewService specialActivityViewService) {
        super(dao, logger, ChurchActivityPlan.class);
    ...
    public ChurchActivityReport getChurchPlansReport(Integer churchId, String startDate, String endDate) throws NotFoundException {
        logger.warn("Retrieving church plans report for id " + churchId + " from " + startDate + " to " + endDate);
        List<ChurchActivityReportView> views = reportDao.findAllByChurchIdAndDateIsBetween(churchId, startDate, endDate);
        ...
        report.setCity(views.get(0).getChurchCity());
        report.setId(views.get(0).getChurchId());
        report.setMaxActivityCount(maxActivityCount - specialActivityCount - worshipCount);
        report.setName(views.get(0).getChurchName());
        ...
        report.setStateAbbrv(views.get(0).getChurchStateAbbrv());
        report.setWorshipCount(worshipCount);

        return sumChurchActivityReport(report, views, null);
    }

    public List<ChurchActivityReport> getChurchPlanReportByCategory(Integer categoryId, Integer churchTypeId,
                                                                    String startDate,
                                                                    String endDate) throws NotFoundException {
        logger.warn("Retrieving church plans report for category id " + categoryId + " from " + startDate + " to " + endDate);
        List<ChurchActivityReportView> views = reportDao.findAllByChurchTypeIdAndDateIsBetween(
                churchTypeId,
                startDate,
                endDate
        );
        ...
        // Organize the plans by church
        Map<Integer, List<ChurchActivityReportView>> churchActivityReportViewMap = new HashMap<>();
        for (ChurchActivityReportView view : views) {
            churchActivityReportViewMap.putIfAbsent(view.getChurchId(), new ArrayList<>());
            churchActivityReportViewMap.get(view.getChurchId()).add(view);
        }

        return churchActivityReportViewMap.values().stream().map(viewsByChurch -> {
            final ChurchActivityReport report = new ChurchActivityReport();
            report.setTotalDays(specialActivityViewService.getTotalDays(startDate, endDate));
            report.setCity(viewsByChurch.get(0).getChurchCity());
            report.setId(viewsByChurch.get(0).getChurchId());
            report.setMaxActivityCount(maxActivityCount - specialActivityCount - worshipCount);
            report.setName(viewsByChurch.get(0).getChurchName());
            report.setRequiredActivityCount(requiredActivityCount - specialRequiredActivityCount - worshipCount);
            report.setSpecialActivityCount(specialActivityCount);
            report.setStateAbbrv(viewsByChurch.get(0).getChurchStateAbbrv());
            report.setWorshipCount(worshipCount);
            report.setType(viewsByChurch.get(0).getChurchType());
            report.setTypeId(viewsByChurch.get(0).getChurchTypeId());

            return sumChurchActivityReport(report, viewsByChurch, categoryId);
        }).collect(Collectors.toList());
    }

    public List<Map<String, Object>> getDailyChurchPlans(Integer churchId, String startDate, String endDate, List<String> columns) {
        logger.warn("Retrieving display church plans for id {} from {} to {}", churchId, startDate, endDate);
        return getAllDisplayFromList(dao.findAllByChurchIdAndDateIsBetween(churchId, startDate, endDate), columns);
    }

    public ChurchActivityReport sumChurchActivityReport(ChurchActivityReport report,
                                                        List<ChurchActivityReportView> views, Integer categoryId) {
        ...
        report.setCategories(new ArrayList<>(categoryReportHashMap.values())
                ...
                .filter(activityCategoryReport -> {
                    ...
                    return activityCategoryReport.getId() == categoryId; })
    ...
    public ActivityCategoryReport getActivityCategoryReportFromView(ChurchActivityReportView view) {
        ...
        activityCategoryReport.setTextColor(view.getCategoryTextColor());
